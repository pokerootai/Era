...
struct NowPlayingView: View {
    ...
    private func content(_ song: LocalSong) -> some View {
        VStack(spacing: 0) {
        VStack(spacing: 24) {
            Artwork(song: song, radius: 28).frame(maxWidth: 240).aspectRatio(1, contentMode: .fit).padding(.horizontal, 26).padding(.top, 8).scaleEffect(player.isPlaying ? 1 : 0.91).animation(.spring(response: 0.45), value: player.isPlaying)
    ...
    private func control(_ icon:String, active:Bool, action:@escaping()->Void)->some View {
        Button(action: action) {
            Image(systemName: icon).font(.title3)
                .foregroundStyle(active ? EraTheme.accent : .primary)
                .contentTransition(.symbolEffect(.replace))
